...
impl Database {
    ...
    pub(crate) fn create_tables_on_conn(conn: &Connection) -> Result<(), AppError> {
        ...
        // 19. Chat Sessions 表 (Hermes Chat)
        conn.execute(
        ...
        // 20. Chat Messages 表 (Hermes Chat)
        conn.execute(
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    10 => {
                        log::info!("迁移数据库从 v10 到 v11（添加 Hermes Chat 支持）");
                        Self::migrate_v10_to_v11(conn)?;
                        Self::set_user_version(conn, 11)?;
                    }
                    11 => {
                        log::info!("迁移数据库从 v11 到 v12（Skills 常用标记支持）");
    ...
    fn migrate_v10_to_v11(conn: &Connection) -> Result<(), AppError> {
        ...
        .map_err(|e| AppError::Database(format!("创建 chat_sessions 表失败: {e}")))?;

        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_chat_sessions_updated ON chat_sessions(updated_at DESC)",
            [],
        )
        .map_err(|e| AppError::Database(format!("创建 chat_sessions 索引失败: {e}")))?;
        ...
        .map_err(|e| AppError::Database(format!("创建 chat_messages 表失败: {e}")))?;

        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_chat_messages_session ON chat_messages(session_id, created_at ASC)",
            [],
        )
        .map_err(|e| AppError::Database(format!("创建 chat_messages 索引失败: {e}")))?;

        log::info!("v10 -> v11 迁移完成：已添加 Hermes Chat 表");
        Ok(())
    }
