...
function normalizeSkill(raw: Record<string, unknown>): ClawHubSkill {
  ...
  const latestVersion = raw.latestVersion as
    | Record<string, unknown>
    | undefined;
  const slug = String(skill.slug ?? "");
  const ownerHandle =
    String(raw.ownerHandle ?? skill.ownerHandle ?? "").trim() || undefined;
  ...
  return {
    ...
    tags: Array.isArray(skill.tags)
      ? Object.keys(skill.tags as object)
      : undefined,
...
export const localRepoApi = {
  async listPublic(
    opts: { cursor?: string; numItems?: number; sort?: string } = {},
  ): Promise<ClawHubSearchResult> {
    ...
    const raw = (await localQuery("skills:listPublicPageV4", args)) as {
      page?: unknown[];
      nextCursor?: string;
      hasMore?: boolean;
    };
...
export const clawHubApi = {
  async listPublic(
    ...
    const raw = (await clawHubQuery("skills:listPublicPageV4", args)) as {
      page?: unknown[];
      nextCursor?: string;
      hasMore?: boolean;
    };
  ...
  async getBySlug(slug: string): Promise<ClawHubSkill | null> {
    const raw = (await clawHubQuery("skills:getBySlug", { slug })) as Record<
      string,
      unknown
    > | null;
  ...
  async publish(opts: {
    ...
    for (const f of opts.files) {
      const binary = Uint8Array.from(atob(f.contentBase64), (c) =>
        c.charCodeAt(0),
      );
      form.append("files", new Blob([binary], { type: f.contentType }), f.path);
    }
    const res = await fetch(
      `${CONVEX_URL.replace(".cloud", ".site")}/api/v1/skills`,
      {
        method: "POST",
